 ...
 */
/* Modified by Ming Liu 27 Feb 2018
 * Random number generator is replaced by dsfmt
 * This file also include uniform distritbution (returns original dsfmt random number between 0 and 1)
 * DO NOT include dsfmt.h twice
 */
/* declaring the global variables (for dsfmt random number generator) */
	dsfmt_t dsfmt;
/* pick which underlying uniform random number generator to use. */
#define DRAND() (dsfmt_genrand_open_open(&dsfmt))
// ((random()+0.5)/ 2147483648.0)
// dsfmt_genrand_open_open(&dsfmt)
// ((dsfmt_genrand_open_open(&dsfmt)+(2147483648))/4294967295)
// ((dsfmt_genrand_open_open(&dsfmt)/(4294967295.0))+(0.5))
