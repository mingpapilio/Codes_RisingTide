 ...
 * This file counts the outcome of repeated simulations 
 * File instruction:
...
gcc ddmc_rep.c gen_beta.h gen_beta.c -lm
./a.out
...
// Global variables
    // Switches
        int s1= 2;                  // Specify the type of performance curve (1: normal dist, 2: beta dist)
        int s2= 2;                  // Specify the type of distribution in environmental conditions
...
int main (void)
{
	...
	// Output
        ...
        fprintf(out,"shape_ris\tshape_bet\tlong_var\tshort_var\tdom_1\tdom_2\tcoexist\n");
    // Starting to list all param space    
    for (c1=1; c1<=5; c1++){
            if(s2==2) shape_long_var=       1000*pow(10, -0.5*(c1-1));
            if(s2==1) sd_tmp=               0.5+ 5*(c1-1);
        for (c2=1; c2<= 5; c2++){
                if(s2==2) shape_short_var=  1000*pow(10, -0.5*c2);
                if(s2==1) sd_short_tmp=     0.5+ 5*(c2-1);
                // reset
            ...
            for(j=1; j<=N_gen; j++){   // avoiding misscounting or other errors
                ...
                // Calculating the populatoin growth parameters
                ...
                tmp_x= p[i_ris];
                tmp_y= p[i_bet];
                if(K_1> 0 && tmp_x> 0) abf_ris= (1+ b_1*(1- tmp_x/K_1- a_br*tmp_y/K_1)- d_1);
                else abf_ris= (1+ b_1- d_1);
                if(K_2> 0 && tmp_y> 0) abf_bet= (1+ b_2*(1- tmp_y/K_2- a_rb*tmp_x/K_2)- d_2);
                else abf_bet= (1+ b_2- d_2);
